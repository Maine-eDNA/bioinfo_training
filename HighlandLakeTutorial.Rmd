...
``` {R phyloseqEnvironment}

# Run the following four lines only if you lost connection to CyVerse
taxa <- read.csv("ASV_taxa.csv", header=T, row.names=1)
head(taxa) ##859 of 8 obs when read in (verses character vector)
seqtab.nochim <- read.csv("seqtab-nochim.csv", header=T, row.names=1)
head(seqtab.nochim) 

#Run these lines no matter what
metadata <- read.table("info_18S.txt", sep = "\t", header = T)
head(metadata)
rownames(metadata)
...
#remove any ASVs that appear <50x per sample
ps_filt_samples <- prune_samples(sample_sums(rawotus)>=500, rawotus)
